 ...
 ## Supervised Learning
	 ...
	 3.  ***Linear Regression***
		 ...
		 3.  Linear regression fits a line to build a model. There are infinite number of lines that can be drawn through the training points. Which line should we consider as the model
			 ...
			 3. The Sum of Squares Regression is the sum of the squared differences between the prediction for each observation and the population mean. = $= \sum{(predicted \ Y - Mean \ of\ Y)}^{2}$
			 4. 
